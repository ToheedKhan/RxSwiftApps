...
class UserListViewController: UIViewController {
    ...
    let userList = BehaviorRelay<[UserDetailViewModel]>(value: [])
    let filteredList = BehaviorRelay<[UserDetailViewModel]>(value: [])
    var controller: UserDetailViewController?
    
    override func viewDidLoad() {
        super.viewDidLoad()
        userViewModelInstance.fetchUserList()
        controller = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(identifier: "UserDetailViewController") as UserDetailViewController
        bindUI()
    }
    ...
    func bindUI() {
        ...
        filteredList.bind(to: tableview.rx.items(cellIdentifier: "CellIdentifier", cellType: UserListCell.self)) { row, model, cell in
    ...
    //Search function
    func filterUserList(userModel: UserDetailViewModel, searchText: String?) -> Bool {
